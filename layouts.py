...
def build_graph(user, server, s2u_idx, u2s_index, u2u_idx, s2s_idx, u2u_distance, u2s_path_loss, u2s_path_loss_feat, env_len):
    ...
    # 我自己加的归一化
    # user_feat = F.normalize(user_feat, dim = 1, p = 1)
    # server_feat = F.normalize(server_feat, dim = 1, p = 1)
    # 归一化结果 sum / len = 0.5
    user_feat = user_feat / sum(user_feat) * len(user_feat) * 0.5
    server_feat = server_feat / sum(server_feat) * len(server_feat) * 0.5
    # u2s_path_loss_feat =
...
def generate_layouts(user_nums, server_nums, args):
    ...
    for idx in range(len(server_nums)):
        ...
        # 让计算资源在 0.1-1, 防止溢出
        server_idx_comp = np.random.uniform(0.1, 1, (server_nums[idx], 1))
        ...
        # variate mask is used to create edge_index
        ...
        u2s_path_loss = compute_path_losses(args, u2s_distances_idx)  # 后续没用上

        # generate normalized channels randomly
        # path_loss 在 0.1-1, 防止溢出
        # u2s_path_loss_feat = torch.rand((user_nums[idx], server_nums[idx]), device=args.device)
        u2s_path_loss_feat = torch.from_numpy(
            np.random.uniform(0.1, 1, (user_nums[idx], server_nums[idx]))).to(args.device)
...
if __name__ == '__main__':
    from arguments import args
    generate_layouts([20, 30], [5, 8], args)
