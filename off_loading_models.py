...
class TaskLoad(nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict, edge_attr_dict):
        ...
        # task_allocation = self.judge(task_allocation)
...
class PCNet(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, args) -> None:
    ...
    def forward(self, u2s_path_loss, user_tasksize, server_comp_resource, edge_index, output_len):
        '''
            input: path_losses of users to servers
            output: power_sche, task_sche, comp_sche
        '''
        self.output_dim = output_len
        now_input_len = u2s_path_loss.shape[1] + user_tasksize.shape[1] + server_comp_resource.shape[1]
        pad = torch.zeros((u2s_path_loss.shape[0], self.input_dim - now_input_len)).to(args.device)
        feats = torch.cat([u2s_path_loss, user_tasksize, server_comp_resource, pad], dim=1)
        ...
        comp_sche = sches[:, 2 * self.output_dim: 3 * self.output_dim]
